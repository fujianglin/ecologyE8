 ...
 * 采购池申请流程推送SAP生成PR
 */
public class PurchaseCaigouAction implements Action {
    //日志打印对象
...
//  private String urlRoot1 = "https://e600086-iflmap.hcisbt.cn1.hana.ondemand.com/cxf/OAtoS4HC_PurReqCreate_TEST";
    ...
    public String execute(RequestInfo requestInfo) {
        ...
        try{
            ...
            StringBuffer sqlUpdate = new StringBuffer();//获取所有采购池中对应的数据id
            String wuliaotext="";//物料短文本
            String caigouID="";//采购池数据的id
            String wuLiaonumber="";//采购池中的物料编号
            String shuliang="";//数量
            //流程requestid
            ...
            while (recordSet.next()){
             ...
             /*   if("".equals(jlDanwei) ||"1".equals(cglx) ){
                	 purchaseRequisitionCreateItem.setBaseUnit("EA");//基本计量单位
                }else{
                	purchaseRequisitionCreateItem.setBaseUnit(Util.null2String(recordSet.getString("BaseUnit")));//基本计量单位
                }*/
                if("1".equals(cglx) ){
               	 purchaseRequisitionCreateItem.setBaseUnit("EA");//基本计量单位
               }else{
               ...
               //判断交货日期如果小于0
                ...
                zz=zz+10;
                //wuliaotext=Util.null2String(recordSet.getString("PurchaseRequisitionItemText"));//获取流程中的物料短文本
                if(Util.null2String(recordSet.getString("PurchaseRequisitionItemText")).length()>40){
                	wuliaotext=Util.null2String(recordSet.getString("PurchaseRequisitionItemText")).substring(0,40);//获取流程中的物料短文本
                }else{
                	wuliaotext=Util.null2String(recordSet.getString("PurchaseRequisitionItemText"));//获取流程中的物料短文本
                }
                ...
                wuLiaonumber=Util.null2String(recordSet.getString("Material"));//获取流程中的物料编号
                shuliang=Util.null2String(recordSet.getString("RequestedQuantity"));//申请数量
                /* 
                String selectIdSql="select id from uf_cgc where materialtext='"+wuliaotext+"' and wlbm='"+wuLiaonumber+"' and quantity='"+shuliang+"'";
                rs1.execute(selectIdSql);
                logger.info("获取采购池id："+selectIdSql);
                if(rs1.next()){
                	caigouID=rs1.getString("id");        	
                }
                */
                caigouID = recordSet.getString("cgcsjid"); //cgcsjid  采购池数据ID 
                
                if(ii==0){	
                	sqlUpdate.append(caigouID);
                }else{
                	sqlUpdate.append(","+caigouID);
                }
            ...
            if(xml.equals("")){
                ...
                //如果是从采购池出来的数据，需要修改一下采购池的状态： 物料短文本 materialtext，物料编号 wlbm，申请人 sqr
                String[] idStrArray=sqlUpdate.toString().split(",");//获取需要更新采购池id的数据
        		for(int i=0;i<idStrArray.length;i++){		
        			String UpdateSql="update uf_cgc set lczt='2',prid ='"+PurchaseRequisition+"'where  id ='"+idStrArray[i]+"'";
                	logger.info("更新采购池sql："+UpdateSql);
