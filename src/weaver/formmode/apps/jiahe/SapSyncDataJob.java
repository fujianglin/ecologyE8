...
public class SapSyncDataJob extends BaseCronJob {
	...
	public Map<String, SapBugetDto> getWbsBudgetData(String wsdl,
			String username, String password, String years) {
		...
		for (int m = 0; m < yearArray.length; m++) {
			...
			if (year.length() > 0) {
				...
				String data = HttpsUtil.httpsRequest(wsdl, "POST", xml,
						username, password);
				...
				for (int i = 0; i < YY1_PROJECT_API_Array.size(); i++) {
					Element YY1_PROJECT_API = (Element) YY1_PROJECT_API_Array
							.get(i);
					List YY1_PROJECT_APIType_Array = YY1_PROJECT_API
							.elements("YY1_PROJECT_APIType");
					for (int j = 0; j < YY1_PROJECT_APIType_Array.size(); j++) {
						Element YY1_PROJECT_APIType = (Element) YY1_PROJECT_APIType_Array
								.get(j);
						String FiscalYear = Util
								.null2String(YY1_PROJECT_APIType
										.element("FiscalYear") == null ? ""
										: YY1_PROJECT_APIType.element(
												"FiscalYear").getTextTrim());
						String WBSElement = Util
								.null2String(YY1_PROJECT_APIType
										.element("WBSElement") == null ? ""
										: YY1_PROJECT_APIType.element(
												"WBSElement").getTextTrim());
						String WBSDescription = Util
								.null2String(YY1_PROJECT_APIType
										.element("WBSDescription") == null ? ""
										: YY1_PROJECT_APIType.element(
												"WBSDescription").getTextTrim());
						String GLAccount = Util.null2String(YY1_PROJECT_APIType
								.element("GLAccount") == null ? ""
								: YY1_PROJECT_APIType.element("GLAccount")
										.getTextTrim());

						if ((FiscalYear.length() != 0)
								&& (WBSElement.length() != 0)
								&& (GLAccount.length() != 0)) {
							String Key = "W_" + FiscalYear + WBSElement
									+ GLAccount;
							...
							double PlanAmountInGlobalCurrency = 0.0D;
							double ActualAmountInGlobalCurrenc = 0.0D;
							String PlanningCategory = Util
									.null2String(YY1_PROJECT_APIType
											.element("PlanningCategory") == null ? ""
											: YY1_PROJECT_APIType.element(
													"PlanningCategory")
													.getTextTrim());
							String Ledger = Util
									.null2String(YY1_PROJECT_APIType.element(
											"Ledger").getTextTrim());
							String IsCommitment = Util
									.null2String(YY1_PROJECT_APIType.element(
											"IsCommitment").getTextTrim());
							if ("BUDGET01".equals(PlanningCategory)) {
								PlanAmountInGlobalCurrency = Util
										.getDoubleValue(
												YY1_PROJECT_APIType
														.element(
														...
														.getTextTrim(), 0.0D);
								sapBugetDto
										.setPlanAmountInGlobalCurrency(new BigDecimal(
												sapBugetDto
														.getPlanAmountInGlobalCurrency()
														.add(
																new BigDecimal(
																		PlanAmountInGlobalCurrency))
														.setScale(2, 4)
							...
							if (("ACT01".equals(PlanningCategory))
									&& ("0L".equals(Ledger))) {
								ActualAmountInGlobalCurrenc = Util
										.getDoubleValue(
												YY1_PROJECT_APIType
														.element(
																"ActualAmountInGlobalCurrenc")
														.getTextTrim(), 0.0D);
								sapBugetDto
										.setActualAmountInGlobalCurrenc(new BigDecimal(
												sapBugetDto
														.getActualAmountInGlobalCurrenc()
														.add(
																new BigDecimal(
																		ActualAmountInGlobalCurrenc))
														.setScale(2, 4)
	...
	public Map<String, SapBugetDto> getCostCenterData(String wsdl,
			String username, String password, String years) {
		String[] yearArray = years.split(",");
		Map map = new HashMap();
		for (int m = 0; m < yearArray.length; m++) {
			...
			if (year.length() > 0) {
				...
				Envelope.addNamespace("soap",
						"http://schemas.xmlsoap.org/soap/envelope/");
				Element Body = Envelope.element("Body");
				List YY1_CostCenter_API_Array = Body
						.elements("YY1_CostCenter_API");

				for (int i = 0; i < YY1_CostCenter_API_Array.size(); i++) {
					Element YY1_CostCenter_API = (Element) YY1_CostCenter_API_Array
							.get(i);
					List YY1_CostCenter_APIType_Array = YY1_CostCenter_API
							.elements("YY1_CostCenter_APIType");
					for (int j = 0; j < YY1_CostCenter_APIType_Array.size(); j++) {
						Element YY1_CostCenter_APIType = (Element) YY1_CostCenter_APIType_Array
								.get(j);
						String FiscalYear = Util
								.null2String(YY1_CostCenter_APIType.element(
										"FiscalYear").getTextTrim());
						String CostCenter = Util
								.null2String(YY1_CostCenter_APIType.element(
										"CostCenter").getTextTrim());
						String GLAccount = Util
								.null2String(YY1_CostCenter_APIType.element(
										"GLAccount").getTextTrim());

						if ((FiscalYear.length() != 0)
								&& (CostCenter.length() != 0)
								&& (GLAccount.length() != 0)) {
							String Key = "C_" + FiscalYear + CostCenter
									+ GLAccount;
							SapBugetDto sapBugetDto = null;
							if (map.containsKey(Key)) {
								sapBugetDto = (SapBugetDto) map.get(Key);
							...
							double ActualAmountInGlobalCurrenc = 0.0D;
							String PlanningCategory = Util
									.null2String(YY1_CostCenter_APIType
											.element("PlanningCategory")
											.getTextTrim());
							String Ledger = Util
									.null2String(YY1_CostCenter_APIType
											.element("Ledger").getTextTrim());
							String IsCommitment = Util
									.null2String(YY1_CostCenter_APIType
											.element("IsCommitment")
											.getTextTrim());
							if ("PLN".equals(PlanningCategory)) {
								PlanAmountInGlobalCurrency = Util
										.getDoubleValue(
												YY1_CostCenter_APIType
														.element(
																"PlanAmountInGlobalCurrency")
														.getTextTrim(), 0.0D);
								sapBugetDto
	...
	public void execute() {
		...
		String jhrwl = "weaver.formmode.apps.jiahe.SapSyncDataJob";
		RecordSet jhrwRs = new RecordSet();
		String jhrwZtSql = "select sfqy from uf_sapJhControl where jhrwl='"
				+ jhrwl + "'";
		...
		if (jhrwRs.next()) {
			...
			if ("0".equals(sfqy)) {
				...
				String CostCentwsdl = Util.null2String(Prop.getPropValue(
						"jhsapbrowser", "CostCentUrl"));
				String WbsBudgetwsdl = Util.null2String(Prop.getPropValue(
						"jhsapbrowser", "WBSBudgetUrl"));
				String username = Util.null2String(Prop.getPropValue(
						"jhsapbrowser", "username"));
				String password = Util.null2String(Prop.getPropValue(
						"jhsapbrowser", "password"));
				String currentYear = DateHelper.getCurrentYear();
				String years = Util.getIntValue(currentYear, 0) - 1 + ","
						+ currentYear + ","
						+ (Util.getIntValue(currentYear, 0) + 1);
				...
				Map map = getCostCenterData(CostCentwsdl, username, password,
						years);

				Map wbsmap = getWbsBudgetData(WbsBudgetwsdl, username,
						password, years);
				...
				while (it.hasNext()) {
					...
					String sql = "insert into uf_buget(WBSElement,WBSDescription,PlanAmountInGlobalCurrency,keyong,dongjie,ActualAmountInGlobalCurrenc,budgetyear,CostCenter,GLAccount,formmodeid,modedatacreater,modedatacreatertype,modedatacreatedate,modedatacreatetime)values('"
							+ dto.getWBSElement()
							+ "',"
							+ "'"
							...
							+ "',"
							+ "'"
							+ dto.getPlanAmountInGlobalCurrency()
									.setScale(2, 4).toPlainString()
							+ "',"
							+ "'"
							...
							+ "',"
							+ "'"
							+ dto.getActualAmountInGlobalCurrenc().setScale(2,
									4).toPlainString()
							+ "',"
							+ "'"
							+ dto.getBudgetyear()
