...
SWFUpload.prototype.initSWFUpload = function (settings) {
	...
	function replaced_originPlace() {
		...
		else if(window.location.href.indexOf("/govern/skin/") >= 0) {//政务win10桌面上传图片
			tempParent.innerHTML =  getHTML_governImg(0,"84px");
		}
		else{
			tempParent.innerHTML = getHTML("visible", "104px");
		}
	...
	function getHTML_governImg(isVisib, width) {
		return '<div  class="uploadbtn_img" id=' + settings.button_placeholder_id + ' style="opacity:' + isVisib + ';width:' + width + ';height:26px;background-color:#1565a9;color:#fff!important" >图片</div>';
	}
	...
	var getUrl2 = function () {
        ...
        _savefile = "uploadImg_" + curdate + "." + (file_extension || 'jpg');
        // console.log('getUrl_file_extension',file_extension)
        return "/weaver/weaver.formmode.exceldesign.ExcelUploadServlet?method=uploadFile&savefile="+_savefile;
    }
	...
	if(_getBrowserInfo()) {
		...
		if(window.location.href.indexOf('/formmode/exceldesign/excelUploadImg.jsp')>=0){
            newUrl=getUrl2()+'&'+this.getH5Param()
            console.log('newUrl',newUrl);
        }
...
SWFUpload.prototype.uploadComplete = function (file) {
	if(_getBrowserInfo() && window.location.href.indexOf('/workflow/exceldesign/excelUploadImg.jsp')>=0){
		...
		setPicDesc();
	}else if(_getBrowserInfo() && window.location.href.indexOf('/formmode/exceldesign/excelUploadImg.jsp')>=0){
	    file_extension = file.name.split(".")[1];
        jQuery("#imgname").text(_savefile);
        jQuery("[name=imgsrc_2]").val("/filesystem/exceldesign/uploadimg/"+_savefile);
        setPicDesc();
